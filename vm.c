...
static Value hasFieldNative(VM* vm, int argCount, Value* args) {
...
static Value toStringNative(VM* vm, int argCount, Value* args) {
    if (IS_BOOL(args[0])) {
...
void initVM(VM* vm) {
    ...
    defineNative(vm, "hasField", hasFieldNative);
